...
export const handler = async (event) => {
  try {
    const { networkId, programId } = event.pathParameters;
    const { userWithAccess } = JSON.parse(event.body);

    if (!networkId || !programId) {
      return {
        statusCode: 400,
        body: JSON.stringify({ message: "Missing networkId or programId" }),
      };
    }
