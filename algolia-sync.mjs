 ...
 * 
 * Setup:
 * 1. Enable DynamoDB Streams on shelcaster-app table (NEW_AND_OLD_IMAGES)
 * 2. Create this Lambda function in AWS
 * 3. Add ALGOLIA_APP_ID and ALGOLIA_ADMIN_KEY as environment variables
 * 4. Add the DynamoDB Stream as a trigger for this Lambda
 * 5. Ensure Lambda has permissions to read from DynamoDB Streams
 */
...
// Unmarshal DynamoDB record (handle both marshalled and unmarshalled formats)
function unmarshalRecord(image) {
  ...
  for (const [key, value] of Object.entries(image)) {
    ...
    else result[key] = value; // Already unmarshalled
  }
  return result;
}
